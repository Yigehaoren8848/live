...
def revoi(key, iv):
    js_code = """var CryptoJS = CryptoJS || function(u, l) {
        var d = {}
          , n = d.lib = {}
          , p = function() {}
          , s = n.Base = {
    ...
    (function() {
        ...
        function getCanvas() {
            ...
            renderer = 'YSP' + renderer.replace(/\s*/g, '');
            return renderer;
        }
    ...
    function getHash() {
            ...
            var des_iv = \"""" + iv + """\"
            var crypto_key = CryptoJS.enc.Base64.parse(des_key);
            var crypto_iv = CryptoJS.enc.Base64.parse(des_iv);
            var message  ='{"mver":"1","subver":"1.2","host":"www.yangshipin.cn/#/tv/home?pid=","referer":"","canvas":"YSPANGLE(Intel,Intel(R)UHDGraphics730(0x00004692)Direct3D11vs_5_0ps_5_0,D3D11)"}'

            var encrypted = CryptoJS.TripleDES.encrypt(message, crypto_key, {
                iv: crypto_iv,
    ...
    s = excutor.call("getHash")
    return s


key = '+t/L0DGtTT+NPJ6MDsympA=='
iv = 'Li/bopYaMmU='
print(revoi(key, iv))
